...
public class Explosion extends Actor {
	private static final float MIN_SPEED = 1.65f, MAX_SPEED = 2.25f, // random
																		// speeds
			...
			WAIT_TIME = 1.2f; // time in seconds to wait at max size before
								// dying
	 ...
	 * 
	 * @param fluctuation
	 *            amount of degrees away from vertical/horizontal. Must be 0 <
	 *            fluctuation < 45
	...
	public float getRadius() {
		return getWidth() / 2; // width and height should always be the same so
	...
	public boolean isColliding(Explosion other) {
		...
		if (state == ExplosionState.EXPLODING && other.getState() == ExplosionState.EXPLODING) { // both
																									// exploding
																									// =
																									// not
																									// colliding
